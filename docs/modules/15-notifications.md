...
> **Wave 3 addition (M7):** three application events added — `NewApplicationNotification` (apply → owner), `ApplicationApprovedNotification` (approve → tenant), `ApplicationRejectedNotification` (reject → tenant); all deep-link to `owner.applications.index` / `tenant.applications.index`, and the recorded reject reason is surfaced on the tenant applications page. Others: shortlist, confirm/cancel/decline, and the M7 "request more info" step stay silent for now.
