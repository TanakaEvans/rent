...
- FR-06 **Settings**: key/value system configuration (`system_settings`).
...
5. **Config change** - When admin changes a system setting, the system writes `system_settings` and clears the relevant cache; then the change applies to marketplace/config consumers.
...
| `system_settings` | id, key (unique), value, group | config |
